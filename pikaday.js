...
}(this, function (moment)
{
    ...
    var hasMoment = typeof moment === 'function',
        ...
        renderTitle = function(instance, c, year, month, refYear, randId)
        {
            ...
            if (c === 0) {
                html += '<button class="pika-prev' + (prev ? '' : ' is-disabled') + '" type="button"><i class="fa fa-caret-left"></i></button>';
            }
            if (c === (instance._o.numberOfMonths - 1) ) {
                html += '<button class="pika-next' + (next ? '' : ' is-disabled') + '" type="button"><i class="fa fa-caret-right"></i></button>';
            }
        ...
        renderTimePicker = function(num_options, selected_val, select_class, display_func, increment_by) {
            ...
            var to_return = '<div class="pika-label">' + display_func(selected_val).substr(0,2) + '<select class="pika-select '+select_class+'">';
        ...
        {
            var to_return = '' +
                (opts.timeLabel !== null ? '<div class="pika-time-label">'+opts.timeLabel+'</div>' : '') +
                renderTimePicker(24, hh, 'pika-select-hour', function(i) {
                        if (opts.use24hour) {
                            return i;
                        } else {
                            var to_return = ((i%12) < 10 ? '0' : '' ) + (i%12) + (i<12 ? ' AM' : ' PM');
                            if (to_return == '00 AM') {
                                return opts.i18n.midnight;
                            } else if (to_return == '00 PM') {
